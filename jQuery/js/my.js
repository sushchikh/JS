...
///// карабкаемся по DOM-дереву: //////////////////////////////////////////////////////////////////////////
	...
	var someElse12 = $('#someClass p:first'); // выбирает элмент по ID и в нем находит первый абзац

	var someElse13 = $('p:even'); // выбирает четные параграфы, можно и не четные — :odd

	var someElse14 = $('p:contains("text to find")'); // выбирает элементы, содержащие в себе text to find

	var someElse15 = $('div:has(p)'); // выбирает div'ы внутри которых есть параграфы

	var someElse16 = $('ul li:nth-child(2)'); // выберет Li в UL по порядковому номеру

	var someElse17 = $('li:empty'); // пустые элементы (без контента)
...
// события / events
	...
	$('.someClass').mouseover(function(event) { // наведение мышки НЕ аналог :hover
		/* Act on the event */
	});

	$('.someClass').hover(function() { // аналог :hover в CSS
...
var someVar = $('p').html(); //  вернет html элемента
$('.someElement').html('new HTMLs tags'); // изменит HTML содержание элемента
...
$('.someElement').offset(); // вернет позицию элмента относительно document'a данные будут в виде объекта {top: 10, left: 30}
$('.someElement').offset({top: 10, left: 30}); // устанавливает координаты объекта относительно document'а
$('.someElement').height(); // вернет высоту элмента за вычетом marginov и borderov
// еще есть width() с ним все аналогично
//памятка
$(window).height(); // высота окна
$(document).height(); // высота HTML документа

var someVar = $('.someElement').innerHeight(); // вернут высоту внутренности элемента, включая padding
someVar = $('.someElement').innerHeight(15); // изменят внтуреннюю высоту элмента

var someVar = $('.someElement').outerHeight(); // вернут высоту внутренности элемента, включая padding, border, margin
someVar = $('.someElement').outerHeight(15); // изменят внтуреннюю высоту элмента
...
// ##       ########    ##    ### #### ##     ##  ######
	...
	// 		$('.img').css({
	// 			'left': $(document).width()-$(document).width()-$('.img').width()*2,
	// 			'top': $(document).height()-$('.img').height()*2});
